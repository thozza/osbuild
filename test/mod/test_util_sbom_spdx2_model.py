...
def test_extracted_licensing_info_normalize_name_for_id(name, expected_str):
    assert ExtractedLicensingInfo._normalize_name_for_id(name) == expected_str
...
def test_extracted_licensing_info__generate_license_id(extracted_text, name):
    extracted_text_hash = hashlib.sha256(extracted_text.encode()).hexdigest()
    expect = f"LicenseRef-{ExtractedLicensingInfo._normalize_name_for_id(name)}-{extracted_text_hash}" \
        if name else f"LicenseRef-{extracted_text_hash}"
    assert ExtractedLicensingInfo._generate_license_id(extracted_text, name) == expect
