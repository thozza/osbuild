...
class ExtractedLicensingInfo():
    ...
    def __init__(self, extracted_text: str, name: Optional[str] = None) -> None:
        ...
        self.name = name
        self.license_ref_id = self._generate_license_id(self.extracted_text, self.name)

    @classmethod
    def _generate_license_id(cls, extracted_text: str, name: Optional[str] = None) -> str:
        """
        ...
        if name is not None:
            return f"LicenseRef-{cls._normalize_name_for_id(name)}-{extracted_text_hash}"
