...
def check_depsolve_api_v1_response(res, expected_pkgs, expected_repos, expected_modules, with_dnf5, with_sbom):
    """
...
def test_depsolve_config_combos(tmp_path, repo_servers, dnf_config, detect_fn):
    ...
    for repo_configs, root_dir, opt_metadata in config_combos(tmp_path, tc_repo_servers):
        with TemporaryDirectory() as cache_dir:
            ...
            check_depsolve_api_v1_response(
                res,
...
def check_dump_search_api_v1_response(res, expected_pkgs_count, pkg_check_fn=None):
    """
...
def pkg_with_no_deps_check_fn(pkg):
    """
    Helper function to check the package with no dependencies, which is only available in the custom repository.
    """
...
def test_dump(tmp_path, repo_servers, dnf_config, detect_fn, test_case):
    ...
    for repo_configs, root_dir, opt_metadata in config_combos(tmp_path, tc_repo_servers):
        with TemporaryDirectory() as cache_dir:
            ...
            assert exit_code == 0
            check_dump_search_api_v1_response(res, test_case["packages_count"], pkg_with_no_deps_check_fn)
